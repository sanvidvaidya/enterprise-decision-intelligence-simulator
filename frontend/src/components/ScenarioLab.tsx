...
import { Sliders, Sparkles, RotateCcw, CheckCircle2, ArrowRight, ShieldCheck } from 'lucide-react';
...
export const ScenarioLab: React.FC<ScenarioLabProps> = ({
  ...
  // Default to highest risk account (e.g. Juniper Labs)
  ...
  // Usage events for this customer
  ...
  // Reset sliders when customer changes
  React.useEffect(() => {
  ...
  const handleCommitIntervention = () => {
    confetti({
      ...
      spread: 70,
      origin: { y: 0.6 },
      colors: ['#0071e3', '#10b981', '#38bdf8'],
    });
    toast.success(
      `Intervention playbook committed for ${currentAssessment.customer.customer_name}! Risk reduced by ${Math.abs(
        scoreDelta
      )} pts.`
    );
  };
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-orange-50 text-orange-700 text-[11px] font-semibold mb-2 border border-orange-200">
            ...
            <span>60FPS REACTIVE SCENARIO LAB</span>
          </div>
          <h2 className="text-2xl md:text-3xl font-bold text-[#2d251e] tracking-tight">
            Intervention Simulation & Risk Turnaround Lab
          </h2>
          <p className="text-xs text-[#786c5c] mt-1">
            Zero-latency browser simulation. Drag sliders to model customer success interventions and observe instant risk reduction.
          </p>
        </div>

        {/* Customer Selector Dropdown */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-[#5e5346]">Select Account:</span>
          <select
            value={selectedCustomerId}
            onChange={(e) => setSelectedCustomerId(e.target.value)}
            className="px-3.5 py-2 rounded-xl bg-[#f8f3e8] border border-[#d6c8ad] text-xs font-semibold text-[#2d251e] shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500 cursor-pointer"
          >
            {assessments.map((a) => (
              <option key={a.customer.customer_id} value={a.customer.customer_id}>
                {a.customer.customer_name} ({a.risk_score} pts {a.risk_level})
              </option>
            ))}
          </select>
        </div>
      ...
      {/* Main Grid: Sliders Controls (Col 1) + Live Score Comparison (Col 2) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left: 60fps Intervention Sliders Card */}
        <div className="lg:col-span-2 cockpit-card p-6 md:p-8 space-y-6">
          <div className="flex items-center justify-between border-b border-[#dfd3bc] pb-4">
            <div>
              <h3 className="text-base font-bold text-[#2d251e]">
                Intervention Levers for {currentAssessment.customer.customer_name}
              </h3>
              <p className="text-xs text-[#786c5c]">
                Adjust operational parameters to simulate CS action plans
              </p>
            </div>
            <button
              onClick={handleResetScenario}
              className="p-1.5 rounded-lg text-[#8f8270] hover:text-[#4d4236] hover:bg-[#e4dac4] transition-colors flex items-center gap-1 text-xs font-medium"
          ...
          {/* Slider 1: Active Users Adoption Recovery */}
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-[#3b3229]">
                1. Adoption Recovery (Active Seats):
              </span>
              <span className="font-bold text-orange-600 mono-num">
                {activeUsersSlider} / {purchasedSeats} seats (
                {((activeUsersSlider / purchasedSeats) * 100).toFixed(0)}% engagement)
              </span>
            </div>
            <input
              type="range"
            ...
            <div className="flex justify-between text-[10px] text-[#8f8270] mono-num">
              ...
              <span>100% Utilization: {purchasedSeats} seats</span>
            </div>
          </div>

          {/* Slider 2: Critical Ticket Resolution */}
          <div className="space-y-2 pt-2 border-t border-[#dfd3bc]">
            <div className="flex items-center justify-between">
              <div>
                <span className="font-semibold text-xs text-[#3b3229]">
                  2. Priority Support Escalation Resolution:
                </span>
                <p className="text-[11px] text-[#786c5c]">
                  Deploy senior engineering taskforce to resolve open P1 critical tickets
                </p>
              </div>
          ...
          {/* Slider 3: Contract Extension Runway */}
          <div className="space-y-2 pt-2 border-t border-[#dfd3bc]">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-[#3b3229]">
                3. Commercial Runway Extension (Buffer Days):
              </span>
            ...
            />
            <div className="flex justify-between text-[10px] text-[#8f8270] mono-num">
              <span>0 days (Current contract date)</span>
              <span>+90 days (Quarter extension)</span>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-[#dfd3bc] flex items-center justify-between">
            <span className="text-xs text-[#786c5c]">
              Contract Value Protected: ${((currentAssessment.contract?.annual_contract_value || 0) / 1000).toFixed(0)}k ARR
            </span>
            <button
            ...
            >
              ...
              <span>Commit Playbook & Save ARR</span>
            </button>
          </div>
        </div>

        {/* Right: Real-time Live Score Gauge & Factor Delta Card */}
        <div className="cockpit-card p-6 flex flex-col justify-between space-y-6">
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-around p-4 rounded-2xl bg-[#ece3cf] border border-[#d6c8ad]/80">
              <div className="text-center space-y-1">
                ...
                <div className="text-2xl font-bold text-[#4d4236] mono-num">
                  {baseScore} pts
                </div>
              ...
              <div className="text-center space-y-1">
                ...
                <div className="text-2xl font-bold text-orange-600 mono-num">
                  {simScore} pts
                </div>
                <span
                ...
                {simulatedAssessment.factors.length === 0 && (
                  <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-center space-y-1">
                    ...
                    <p className="text-[11px] text-emerald-600">
                      All criteria successfully remediated.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="p-3 rounded-xl bg-orange-50/60 border border-orange-200 text-[11px] text-orange-900">
            💡 Calculated instantly in browser memory at 60fps without server roundtrips.
          </div>
        </div>
      </div>
    </div>
